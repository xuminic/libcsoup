...
int bmem_main(void *rtime, int argc, char **argv)
{
	(void)rtime; (void)argc; (void)argv;
...
struct	clicmd	tmem_cmd = {
	"tmem", tmem_main, NULL, "Testing the tiny memory management"
};

struct	clicmd	cmem_cmd = {
	"cmem", cmem_main, NULL, "Testing the doubly linked memory management"
};

struct	clicmd	bmem_cmd = {
	"bmem", bmem_main, NULL, "Testing the bitmap memory management"
};

extern  struct  clicmd  tmem_cmd;
extern  struct  clicmd  cmem_cmd;
extern  struct  clicmd  bmem_cmd;
